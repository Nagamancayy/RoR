...
const configSchema = z.strictObject({
  maxInputBytes: z.literal(127).default(127),
...
function validateInput(input: Buffer) {
  if (input.length > 127)
    throw new DomainError(
      'INPUT_TOO_LARGE',
      'ModifVigne v3.4 accepts at most 127 bytes per query.',
    );
  if (!isUtf8(input))
    throw new DomainError(
...
> = {
  metadata: {
    ...
    description:
      'Original encrypt_v3_4, unchanged. UTF-8 only, maximum 127 bytes. A fixed secret 32-character Base64 key (192 random bits) per round. Each query uses the original os.urandom salt generator and random 128-byte block padding. Payload order: tag (128 bytes), salt (32 bytes), ciphertext (128 × (floor(inputBytes / 128) + 1) bytes).',
    securityNote:
