...
async function start(masterKey) {
  ...
  const child = spawn(process.execPath, ['node_modules/next/dist/bin/next', 'start', '--hostname', '127.0.0.1', '--port', String(port)], {
    env: { ...process.env, NODE_ENV: 'production', ROR_MASTER_KEY: masterKey, DATABASE_URL: `file:${path.join(directory, 'lab.db')}`, NEXT_TELEMETRY_DISABLED: '1' },
...
try {
  for (const key of ['', 'invalid-key']) {
    ...
    try {
      const exited = await Promise.race([server.closed.then(() => true), delay(15000).then(() => false)]);
  ...
  try {
    ...
    for (let attempt = 0; attempt < 100; attempt++) {
      try { ready = (await fetch(`${server.url}/api/algorithms`)).ok; } catch { /* Startup still in progress. */ }
      if (ready || server.child.exitCode !== null) break;
      await delay(100);
    }
    assert.ok(ready, 'Configured production server must become ready.');
    const post = async (route, body) => {
      const response = await fetch(`${server.url}${route}`, { method: 'POST', headers: { 'Content-Type': 'application/json', Origin: server.url }, body: JSON.stringify(body) });
      assert.ok(response.ok, `Production request failed: ${response.status}`);
      return response.json();
    };
    const experiment = await post('/api/experiments', { kind: 'ENCRYPTION_ROR', algorithmId: 'aes-256-gcm', queryLimit: 1 });
    assert.equal(experiment.status, 'ACTIVE');
    assert.equal(Object.hasOwn(experiment, 'world'), false);
    const queried = await post(`/api/experiments/${experiment.id}/query`, { encoding: 'utf8', data: 'production smoke test' });
